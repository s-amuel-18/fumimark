...
class EmailSendController extends Controller
{
    ...
    public function contact_email_async(Request $request)
    {
        $data = request()->validate([
            "nombre" => "required|string",
            "comment" => "required",
        ...
        if (!$dalyEmailsValid) {
            ...
            $response = [
                "success_email_send" => false,
                "message" =>  "Error de envio, se llegó al limite de envios diarios, pedes intentarlo mas tarde."
            ];
        ...
        $info["subject"] =  "Cotizacion de servicios Fumimark";
        $info["body"] =  $data["comment"];
        try {
            ...
            if (auth()->user()) {
                $enviado = auth()->user()->emailEnviado()->attach($newEmail->id);
                (new Contact_email())->groupBySendEmail(auth()->user()->id, $newEmail->id, $info);
            }
    ...
    public function mail_contact_custo(Request $request)
    {
        // dd();
