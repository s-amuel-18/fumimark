function show_loader_sende() {
    ...
    const send_emails_progress = document.getElementById(
        "send_emails_progress"
    );
    const send_emails_progress_text = document.getElementById(
        "send_emails_progress_text"
    );
...
async function send_emails(url = null, params, submiter = null) {
    ...
    const select_alert_new_email = document.getElementById(
        "select_alert_new_email"
    );
    ...
    try {
        ...
        if (data.success_email_send) {
            ...
            if (data.times_last_email) {
                console.log(select_alert_new_email);
